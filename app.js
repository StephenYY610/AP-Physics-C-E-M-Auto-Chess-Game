...
function battle(){state.battleLog.push(`<b>Battle Round ${state.round}</b>`); for(let owner=0; owner<2; owner++){const questionOwner=state.players[owner],responder=state.players[1-owner]; const counts={electrostatics:0,gauss:0,circuits:0,magnetism:0,induction:0,potential:0,differential:0,combined:0}; questionOwner.fielded.forEach(u=>u.tags.forEach(t=>counts[t]++)); let circuitsCorrect=0;
    questionOwner.fielded.forEach(u=>{const ans=prompt(`${questionOwner.name} asks ${responder.name}: ${u.name}\n${u.q}\n${u.choices.join('\n')}\n(${responder.name}: Type A/B/C/D; leave blank for timeout)`,''); let result='timeout'; if(ans!==null&&ans.trim()!==''){result=ans.trim().toUpperCase()===u.a.toUpperCase()?'correct':'wrong';}
      if(result==='correct'){let dmg=Math.floor(BASE_DAMAGE[u.cost].correct*(u.star===2?1.5:u.star===3?2:1)); u.tags.forEach(t=>{const tt=tier(t,counts[t]); dmg+= (SYNERGY_DAMAGE[t]&&SYNERGY_DAMAGE[t][tt])||0; if(t==='magnetism'&&tt===5&&Math.random()<0.3) dmg*=2; if(t==='induction'&&tt===4) dmg+=3; if(t==='induction'&&tt===5) dmg+=Math.floor(dmg*0.4);}); if(u.tags.includes('differential')&&tier('differential',counts.differential)===5) questionOwner.hp-=6; questionOwner.hp-=dmg; if(u.tags.includes('circuits')) circuitsCorrect++; state.battleLog.push(`<span class='good'>${responder.name} answered ${u.name} correctly: -${dmg} HP to ${questionOwner.name}</span>`);} else {const loss=BASE_DAMAGE[u.cost][result]; responder.hp-=loss; state.battleLog.push(`<span class='bad'>${responder.name} ${result} on ${u.name}: -${loss} self HP</span>`);} }); if(tier('circuits',counts.circuits)===4&&circuitsCorrect>=2){questionOwner.hp-=3; state.battleLog.push(`<span class='good'>${responder.name} circuits combo from ${questionOwner.name}'s board: extra 3 damage.</span>`);} }
  settlement();
}
